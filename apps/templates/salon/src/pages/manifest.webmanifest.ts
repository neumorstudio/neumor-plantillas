...
import { getWebsiteConfig } from "@lib/supabase";
...
export const GET: APIRoute = async ({ request }) => {
  ...
  const config = (websiteData?.config ?? {}) as Record<string, unknown>;
  ...
  const businessName =
    ...
    toString((websiteData?.clients as Record<string, unknown> | undefined)?.business_name) ||
    DEFAULT_NAME;

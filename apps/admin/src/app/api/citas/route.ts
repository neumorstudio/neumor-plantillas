...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (getUnknownKeys(rawBody, allowedAppointmentKeys).length > 0) {
      return NextResponse.json(
        { error: "Datos invalidos" },
        { status: 400, headers: (await getFallbackCorsHeaders()) ?? undefined }
      );
    ...
    const servicio = body.servicio.trim().slice(0, 120);
    const profesional = body.profesional?.trim().slice(0, 120) || "";
    ...
    if (bookingError) {
      console.error("Error creando cita:", bookingError);
      return NextResponse.json(

// ============================================
// FEATURE ICONS - Iconos para la sección de Features
// ============================================
...
// Get icon SVG by id
export function getFeatureIconSvg(id: string): string {
  const icon = FEATURE_ICONS.find(i => i.id === id);
  return icon?.svg || FEATURE_ICONS.find(i => i.id === "star")?.svg || "";
}

// Normalize icon: if SVG is passed, try to find matching ID
export function normalizeFeatureIcon(icon: string): string {
  // If already a valid ID, return it
  if (FEATURE_ICONS.some(i => i.id === icon)) {
    return icon;
  }
  // If it's an SVG, find the matching ID
